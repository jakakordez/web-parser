        ...
        private static void Generalize(Element baseRoot, Element root2)
        {
            if (baseRoot.Equal(root2))
            {
                ...
                for (int current1 = 0; current1 < baseRoot.children.Count; current1++)
                {
                    ...
                    {
                        if (current1 > 0 && !(baseRoot.children[current1 - 1] is Optional) && baseRoot.children[current1-1].EqualRecursive(root2.children[current2]))
                        {
                            ...
                            current1 --;
                            while (current1 >= 0 && baseRoot.children[current1].EqualRecursive(baseRoot.children[current1-1]))
                            {
                ...
                {
                    if (!(baseRoot.children[baseRoot.children.Count-1] is Optional) && baseRoot.children[baseRoot.children.Count - 1].EqualRecursive(root2.children[current2]))
                    {
                        if (!(baseRoot.children[baseRoot.children.Count - 1] is Iterator))
                        {
                            ...
                            while (baseRoot.children.Count > 1 && baseRoot.children[baseRoot.children.Count - 1].EqualRecursive(baseRoot.children[baseRoot.children.Count - 2]))
                            {
